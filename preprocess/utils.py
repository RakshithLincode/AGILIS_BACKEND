...
def initial_capture_util(data):
    ...
    for img,cam in zip(img_name_list,cam_name):
        print(img,cam)
        img_url = str(img).replace('/critical_data','http://0.0.0.0:3306/')
...
def set_init_regions_util(data):
    ...
    for reg in regions:
        ...
        if  x1 == x0 or y1 == y0 or x1 < x0 or y1 < y0 :
            message = "incorrect or out of image annotation"
            status_code = 400
            return message,status_code
    """
    #img_url =  data['src']
...
def capture_util(data):
    ...
    for reg in regions:
        ...
        print(x1)
        print(y1)
        if  x1 == x0 or y1 == y0 or x1 < x0 or y1 < y0  or y1 > img_h or x1 > img_w :
            message = "incorrect or out of image annotation"
            status_code = 400
            return message,status_code

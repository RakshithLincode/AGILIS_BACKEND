...
def start_real_inspection(data1,inspection_id):
    ...
    def send_crop(j,width,height,frame):
        ...
        mid_crp = frame[y0_1:y1_1 , x1_1+10:x0_2-10].copy()
    ...
    def black_check(regions,rch,r_key,port):
        ...
        for j in regions:
            ...
            if j["cls"] == "region1" and j+1["cls"] == "region2":
                mid_crp = send_crop(j,width,height,frame)

            elif j["cls"] == "region2" and j+1["cls"] == "region3":
                mid_crp = send_crop(j,width,height,frame)

            elif j["cls"] == "region3" and j+1["cls"] == "region4":
                mid_crp = send_crop(j,width,height,frame)
    ...
    while(True):
                        ...
                        if f['cam_name'] == 'extreme_left_camera':
                            try:
                                ...
                                if regions != "":
                                    ...
                                    #if jig_type == "GVX":
                                    #    green1 = black_check(regions,rch,r_key,8515)
                                    print("&&&&&&&&&&&&&&&")   
                                    print(p1_lst)
                                    print("&&&&&&&&&&&&&&&")
                        ...
                        if f['cam_name'] == 'extreme_right_camera':
                            try:
                                ...
                                if regions != "":
                                    ...
                                    p5_lst = regions_crop_pred(regions,rch,r_key,p5_lst,8515)
        ...
        def launch_all_containers():
            ...
            t1 = time.time()
            command1 = "docker container run --gpus all -p 8511:8501 -v " + "\'" + all_saved_model_pth + "\'" + ":/models/saved_model -e MODEL_NAME=saved_model tensorflow/serving:1.14.0-gpu  --per_process_gpu_memory_fraction=0.15"
            command2 = "docker container run --gpus all -p 8512:8501 -v " + "\'" + all_saved_model_pth + "\'" + ":/models/saved_model -e MODEL_NAME=saved_model tensorflow/serving:1.14.0-gpu  --per_process_gpu_memory_fraction=0.15"
            command3 = "docker container run --gpus all -p 8513:8501 -v " + "\'" + all_saved_model_pth + "\'" + ":/models/saved_model -e MODEL_NAME=saved_model tensorflow/serving:1.14.0-gpu  --per_process_gpu_memory_fraction=0.15"
            command4 = "docker container run --gpus all -p 8514:8501 -v " + "\'" + all_saved_model_pth + "\'" + ":/models/saved_model -e MODEL_NAME=saved_model tensorflow/serving:1.14.0-gpu  --per_process_gpu_memory_fraction=0.15"
            command5 = "docker container run --gpus all -p 8515:8501 -v " + "\'" + all_saved_model_pth + "\'" + ":/models/saved_model -e MODEL_NAME=saved_model tensorflow/serving:1.14.0-gpu  --per_process_gpu_memory_fraction=0.15"
            passwd = '123456789'
            subprocess.Popen(command1,shell=True)
            subprocess.Popen(command2,shell=True)
            subprocess.Popen(command3,shell=True)

...
def get_pred_tf_serving_dup(batch,port,model_type,NUM_CLASSES,PATH_TO_LABELS):
    NUM_CLASSES = int(NUM_CLASSES)
    
    #img_exp_lst = []
    #for b_img in batch:
        #image_expanded = np.expand_dims(b_img, axis=0)
    #    img_exp_lst.append(b_img)
    np_arr = np.asarray(batch)
    ...
    print("BATCH SHAPE : : : : : : : : ", np_arr.shape)
    print(len(np_arr.tolist()))
    print("EXECUTING @@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@")
    ...
    print("RESPONSE JSONNNNN")
    #print(response.json())
    print("END OF REQ JSON")
    ...
    for prediction in response.json()['predictions']:
        ...
        final_ret_obj.append(ret_obj)
    return final_ret_obj
...
def start_real_inspection(data1,inspection_id):
    ...
    def send_crop_pred(j,next_region,width,height,frame,port):
        ...
        ret = get_pred_tf_serving(mid_crp,port,"line",1,black_labelmap_pth)
    ...
    def regions_crop_pred(regions,rch,r_key,final_dct,port):
        ...
        for j in regions:
            ...
            print(crp.shape)
            t1 = time.time()
            ret = get_pred_tf_serving(crp,port,"merged",all_num_classes,all_labelmap_pth)
            scores,boxes,objects,accuracy = ret['scores'], ret['boxes'], ret['objects'], ret['accuracy']
            t2 = time.time()
            print('time taken to predict            ::  ::  ::  :: ---------    '+ str(t2-t1))
            #scores,boxes,objects,accuracy = detect_gvm_frame(crp,detection_graphm,sessm,accuracy_thresholdm,category_indexm)
    ...
    def regions_crop_pred_dup(regions,rch,r_key,final_dct,port):
        global frame
        global crp
        batch = []
        batch_cls_list = []
        
        #for reg_k in region_rkey:
        ...
        batch_ret = get_pred_tf_serving_dup(batch,port,"merged",all_num_classes,all_labelmap_pth)
        for idx,ret in enumerate(batch_ret):
            scores,boxes,objects,accuracy = ret['scores'], ret['boxes'], ret['objects'], ret['accuracy']
            t2 = time.time()
            print('time taken to predict            ::  ::  ::  :: ---------    '+ str(t2-t1))
            #scores,boxes,objects,accuracy = detect_gvm_frame(crp,detection_graphm,sessm,accuracy_thresholdm,category_indexm)
    ...
    while(True):
        ...
        def get_pred_extreme_left_camera(p1_lst,jig_type):
            ...
            for cam,r_key in zip(cam_list,key_list):
                if cam == "extreme_left_camera":
                    for f in full_img:
                        if f['cam_name'] == 'extreme_left_camera':
                            try:
                                regions = f['regions']
                                regionss1 = regions.copy()
                                #region_rkey1[r_key] = regionss1
                                
                                region_rkey.append({r_key:regionss1})
                                ...
                                if regions != "":
                                    ...
                                    p1_lst = regions_crop_pred_dup(regions,rch,r_key,p1_lst,8511)
                                    if jig_type == "GVX":
                                        p1,p2,p3 = black_check(regions,rch,r_key,8515,"region1","region2","region3","region4")
                ...
                elif cam == "left_camera":
                    for f in full_img:
                        if f['cam_name'] == 'left_camera':
                            try:
                                regions = f['regions']
                                #regionss2 = regions.copy()
                                #region_rkey2[r_key] = regionss2
                                region_rkey.append({r_key:regionss2})
                                if regions != "":
                                    ...
                                    #p2_lst = regions_crop_pred(regions,rch,r_key,p2_lst,8512)
                                    if jig_type == "GVX":
                                        p4,p5,p6 = black_check(regions,rch,r_key,8515,"region5","region6","region7","region8")
            ...
            #finreg = regionss1 + regionss2 
            #print("LENGTH OF REGION IS ",len(finreg))
            #p1_lst = regions_crop_pred_dup(finreg,rch,r_key,p1_lst,8511,region_rkey)    
            #var = str(inspection_id) + "_cam1"
            #rch.set_json({var:p1_lst}) 
                        ...
                        if f['cam_name'] == 'middle_camera':
                            try:
                                ...
                                if regions != "":
                                    ...
                                    p3_lst = regions_crop_pred(regions,rch,r_key,p3_lst,8513)
                                    if jig_type == "GVX":
                                        p1,p2,p3 = black_check(regions,rch,r_key,8515,"region9","region10","region11","region12")
                        ...
                        if f['cam_name'] == 'right_camera':
                            try:
                                ...
                                if regions != "":
                                    ...
                                    p4_lst = regions_crop_pred(regions,rch,r_key,p4_lst,8514)
                                    if jig_type == "GVX":
                                        p1,p2,p3 = black_check(regions,rch,r_key,8515,"region13","region14","region15","region16")
                        ...
                        if f['cam_name'] == 'extreme_right_camera':
                            try:
                                ...
                                if regions != "":
                                    ...
                                    p5_lst = regions_crop_pred(regions,rch,r_key,p5_lst,8514)
                                    if jig_type == "GVX":
                                        p1,p2 = black_check(regions,rch,r_key,8515,"region17","region18","region19",None)
        ...
        def launch_all_containers():
            ...
            print("LAUNCHING ALLLLLLLLL CONTAINERS")

            t1 = time.time()
            path_f = "/home/schneider/Music"
            command1 = "docker container run --gpus all  -p 8511:8501 -v " + "\'" + all_saved_model_pth + "\'" + ":/models/saved_model -v " + "\'" + path_f + "/batch.config" + "\'" + ":/models/batch.config" +" -e MODEL_NAME=saved_model tensorflow/serving:1.14.0-gpu --enable_batching=true --batching_parameters_file=/models/batch.config"
            
            print(command1)
            command2 = "docker container run --gpus all -p 8512:8501 -v " + "\'" + all_saved_model_pth + "\'" + ":/models/saved_model -e MODEL_NAME=saved_model tensorflow/serving:1.14.0-gpu  --per_process_gpu_memory_fraction=0.15 --tensorflow_intra_op_parallelism=8 --tensorflow_inter_op_parallelism=8"
            command3 = "docker container run --gpus all -p 8513:8501 -v " + "\'" + all_saved_model_pth + "\'" + ":/models/saved_model -e MODEL_NAME=saved_model tensorflow/serving:1.14.0-gpu  --per_process_gpu_memory_fraction=0.15 --tensorflow_intra_op_parallelism=8 --tensorflow_inter_op_parallelism=8"
            command4 = "docker container run --gpus all -p 8514:8501 -v " + "\'" + all_saved_model_pth + "\'" + ":/models/saved_model -e MODEL_NAME=saved_model tensorflow/serving:1.14.0-gpu  --per_process_gpu_memory_fraction=0.15 --tensorflow_intra_op_parallelism=8 --tensorflow_inter_op_parallelism=8"
            command5 = "docker container run --gpus all -p 8515:8501 -v " + "\'" + black_saved_model_pth + "\'" + ":/models/saved_model -e MODEL_NAME=saved_model tensorflow/serving:1.14.0-gpu  --per_process_gpu_memory_fraction=0.15 --tensorflow_intra_op_parallelism=8 --tensorflow_inter_op_parallelism=8"
            passwd = '123456789'
            subprocess.Popen(command1,shell=True)
            #subprocess.Popen(command2,shell=True)
            #subprocess.Popen(command3,shell=True)
            ...
            time.sleep(10)
            print("all containers launched")
            t2 = time.time()
            print('all docker making it up and running time is            ::  ::  ::  :: ---------    '+ str(t2-t1))
        ...
        for i in container_ids:
            i = i.decode("utf-8")
            if '0.0.0.0:8511' in i:
                gvx_gvm_lst.append("8511")
            elif '0.0.0.0:8512' in i :
                gvx_gvm_lst.append("8512")
            elif '0.0.0.0:8513' in i:
                gvx_gvm_lst.append("8513")
            elif '0.0.0.0:8514' in i:
                gvx_gvm_lst.append("8514")
            elif '0.0.0.0:8515' in i:
                gvx_gvm_lst.append("8515")
            """

...
#from livis.gvmmodel import detect_gvm_frame
#from livis.gvxmodel import detect_gvx_frame
import datetime
import base64
import numpy as np
import requests
...
def get_pred_tf_serving(image,port):
    ...
    #ret, frame = cv2.imencode('.jpg', frame1)
    data = json.dumps({ 
...
def start_real_inspection(data1,inspection_id):
    ...
    for cam in data['camera_config']['cameras']:
        ...
        key = RedisKeyBuilderServer(workstation_id).get_key(cam['camera_id'],'original-frame') #WS-01_0_original-frame
    ...
    def regions_crop_pred(regions,rch,r_key,final_dct,port):
        ...
        #cv2.imwrite('/critical_data/a.jpg',frame1)
        #frame = cv2.imread('/critical_data/a.jpg')
        height,width,c = frame.shape
        #print("recieved crop of size : : : : ", frame.shape)
        #height = height*3
        #width = width*3
        for j in regions:
    ...
    while(True):
        ...
        #clear out previous frame values
        final_dct = {}
        #print(full_img)
        #print(cam_list)
        #print(key_list)
